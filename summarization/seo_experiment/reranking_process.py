...
def rerank(options):
    logger.info("creating features")
    create_index(options.trectext_file,os.path.dirname(options.new_index),os.path.basename(options.new_index),options.home_path,options.indri_path)
    features_file = create_features_file_diif(options.features_dir, options.merged_index,options.new_index, options.queries_file,
                                         options.new_features_file, options.workingset_file, options.scripts_path)
    logger.info("creating docname index")
    docname_index = create_index_to_doc_name_dict(features_file)
    logger.info("docname index creation is completed")
